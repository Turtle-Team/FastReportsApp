...
@HiltViewModel
class ReportFolderItemViewModel @Inject constructor(private val fastReportRepository: FastReportRepository) :
    ViewModel() {
    ...
    fun getFolderById(id: String) = viewModelScope.launch(Dispatchers.IO) {
        val list = fastReportRepository.getFolderReportsByid(id).files
